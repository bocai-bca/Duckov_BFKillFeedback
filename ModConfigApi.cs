...
public static class ModConfigAPI
{
	...
	public static string TAG = $"ModConfig_v{ModConfigVersion}";
	...
	public static bool SafeAddInputWithSlider(string modName, string key, string description, Type valueType, object defaultValue, UnityEngine.Vector2? sliderRange = null)
	{
		...
		try
		{
			...
			object[] parameters = sliderRange.HasValue ?
				...
				new object[] { modName, key, description, valueType, defaultValue, Vector2.zero };
	...
	public static T SafeLoad<T>(string mod_name, string key, T defaultValue)
	{
		key = $"{mod_name}_{key}";
		if (!Initialize())
			return defaultValue;
